...
int main() {
    ...
    GLFWwindow *window = glfwCreateWindow(800, 600, "Example12", nullptr, nullptr);
    if (window == nullptr) {
        glfwTerminate();
        std::exit(EXIT_FAILURE);
    }
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        const glm::mat4 model = glm::rotate(glm::mat4(1.0f), static_cast<float>(glfwGetTime()) * glm::radians(-45.0f), glm::vec3(1.0f, 1.0f, 0.0f));
